...
The GOG release is mostly the same as the Steam version, but with Steam DRM removed, all languages included, `LARGEADDRESSAWARE` enabled, and a strange extra inaccurate frame limiter added, with not much else going for it. This probably goes without saying, but you should check out [Juiced Patch](https://github.com/kobraworksmodding/Saints-Row-2-Juiced-Patch), where we address many of these issues and disable GOG's frame-rate limiter if you're playing the GOG version, and even patch the executable to enable the `LARGEADDRESSAWARE` flag if playing on non-GOG releases.